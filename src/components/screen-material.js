...
Pixel getPixel() {
  ...
  // get the UV relative to the pixel
...
float getLedFactor(vec2 uv, float ledWidth, float ledPos, float ledPow) {
  ...
  float ledFactor = step(distanceFromLed, ledWidth);
  ledFactor = valueRemap(distanceFromLed, ledWidth - ledWidth * 0.8, ledWidth, 1.0, 0.);
  ledFactor = clamp(ledFactor, 0., 1.);
  ledFactor = pow(ledFactor, ledPow);
  return ledFactor;
}
